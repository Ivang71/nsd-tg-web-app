...
// const debounce = <F extends (...args: any[]) => any>(func: F, delay: number): (...args: Parameters<F>) => any => {
...
class MetaStore {
  ...
  async getInitialData() {
    ...
    const p = [
      fetch(baseApiUrl + '/products/categories?per_page=100').then((r) => r.json()).then((c) => this.categories = c),
    ]
    // for (let i = 1 i < 4 i++) {
    ...
    this.categories = this.categories
      .filter(c => c.name !== 'Снюс' && c.name !== 'Misc')
